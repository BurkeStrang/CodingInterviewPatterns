function floodFill(grid, sr, sc, target) {
    ...
    if (grid[sr][sc] === target) {
        ...
        // Storing the original value of the starting cell in `oldTarget`,
...
function dfs(grid, row, col, oldTarget, newTarget) {
    ...
    // iii). [-1, 0] -> move up
    // iv).  [0, -1] -> move left
    var adjacentCells = [[0, 1], [1, 0], [-1, 0], [0, -1]];
    ...
    for (var i = 0; i < adjacentCells.length; i++) {
        ...
        if (
            r < gridLength && r >= 0 && c < totalCells &&
            c >= 0 && grid[r][c] === oldTarget
        ) {
...
function main() {
    ...
    for (let i = 0; i < grids.length; i++) {
        ...
        console.log("\t Starting row and column are: (" , startingRow[i], ", ", startingCol[i], ")")
        console.log("\t Target value: ", newTarget[i])
        console.log("\t After perform flood fill: ", floodFill(grids[i], startingRow[i], startingCol[i], newTarget[i]))
