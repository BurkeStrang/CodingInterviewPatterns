...
function backtrack(s, prevDot, dots, segments, result) {
  ...
  for (var currDot = prevDot + 1; currDot < maxPos; currDot++) {
    ...
    if (valid(segment)) {
      ...
      if (dots - 1 == 0) updateSegment(s, currDot, segments, result);
      else backtrack(s, currDot, dots - 1, segments, result); // continue to place dots
...
function main() {
  ...
  for (let i = 0; i < ipAddresses.length; i++) {
    console.log(i + 1 + `.\t Input addresses: '${ipAddresses[i]}'`);
    console.log(
